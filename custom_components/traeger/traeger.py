...
from .model import Acc, Details, Device, Features, Limits, Settings, Status, Thing, User
...
def ensure_string(value: str | bytes) -> str:
...
class traeger:  # pylint: disable=invalid-name,too-many-instance-attributes,too-many-public-methods
    ...
    def set_callback_for_grill(self, grill_id: str, callback: AccessoryUpdateCallback) -> None:
        ...
        self.grill_callbacks.setdefault(grill_id, []).append(callback)
    ...
    async def __get_mqtt_client(self) -> None:
        ...
        headers = {
            "Host": f"{ensure_string(mqtt_parts.netloc)}",
        }
        self.mqtt_client.ws_set_options(
            path=f"{ensure_string(mqtt_parts.path)}?{
                ensure_string(mqtt_parts.query)}",
            headers=headers,
        )
